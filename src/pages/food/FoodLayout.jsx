 ...
 *
 * FIXED (2026-03-28):
 *  - "← Admin Panel" button now only shows when the user is an admin bridge
 *    (sessionStorage flag set by FoodRoute), not for all users including FOOD_MANAGER.
 *  - Name/role now reads from sessionStorage bridge values when admin-bridged,
 *    falls back to localStorage food keys.
    ...
    return (
        <div style={{ display: "flex", height: "100vh", overflow: "hidden", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
            ...
            <aside style={{
                ...
                <div style={{ padding: "22px 20px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        <img src="/main.webp" alt="Logo" style={{ height: "36px" }} />
                        <div>
                            <div style={{ color: "#f1f5f9", fontWeight: 700, fontSize: "0.85rem" }}>VTU HABBA</div>
                            <div style={{ color: "#d4af37", fontSize: "0.7rem", fontWeight: 600 }}>🍽️ Food Portal</div>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ padding: "14px 20px", borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", flexDirection: "column", gap: "8px" }}>
                    {/* Only show "← Admin Panel" for admin-bridged sessions */}
                    {isBridge && (
                        <Link to="/ad-dashboard" style={{
            ...
            <main style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", background: "#0f172a" }}>
                {/* Top bar */}
                <div style={{
                    padding: "14px 26px", borderBottom: "1px solid rgba(255,255,255,0.08)",
                    background: "rgba(15,23,42,0.8)", backdropFilter: "blur(10px)",
                    display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0,
                }}>
                    <h2 style={{ margin: 0, color: "#f1f5f9", fontSize: "1.05rem", fontWeight: 600 }}>
                        {currentItem ? `${currentItem.icon} ${currentItem.label}` : "🍽️ Food Management"}
                    </h2>
                    <span style={{ color: "#64748b", fontSize: "0.78rem" }}>VTU Youth Fest 2026</span>
                </div>

                {/* Content */}
                <div className="dashboard-glass-wrapper" style={{ flex: 1, overflowY: "auto", padding: "20px 28px" }}>
                    {children}
                </div>
            </main>
        </div>
