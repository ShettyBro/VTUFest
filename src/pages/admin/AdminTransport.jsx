...
const Stat = ({ label, value, color }) => (
    <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", padding: "14px 20px", minWidth: "140px", textAlign: "center" }}>
...
function NoteEditor({ row, onSaved, onClose }) {
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await adminFetch(`${API_BASE}/api/admin/transport/note/${row.id}`, {
                method: "PATCH",
...
const COLS = [
    ...
    { key: "contact_person_name",        label: "Contact",       align: "left"   },
    { key: "coordination_status",        label: "Status",        align: "center" },
    { key: "driver_name",                label: "Driver",        align: "left"   },
    { key: "internal_note",              label: "Note",          align: "left"   },
    { key: "actions",                    label: "Actions",       align: "center" },
];
const NON_SORTABLE = new Set(["coordination_status", "driver_name", "internal_note", "actions"]);

// ─── CSV Export ────────────────────────────────────────────────────────────────
function exportCSV(rows) {
    const headers = ["College", "Code", "Mode", "Vehicle/Train No", "PNR", "Arrival Point", "ETA", "Headcount",
        "Contact Name", "Contact Phone", "On-Vehicle Contact", "On-Vehicle Phone",
        "Additional Notes", "Status", "Internal Note", "Driver Name", "Driver Phone", "Vehicle Number", "Driver Remarks", "Submitted At"];
    ...
    const lines = [
        ...
        ...rows.map((r) => [
            ...
            r.driver_name, r.driver_phone, r.vehicle_number, r.driver_remarks,
    ...
    a.href = url; a.download = "admin_transport_submissions.csv"; a.click();
...
export default function AdminTransport() {
    ...
    const fetchData = () => {
        ...
        adminFetch(`${API_BASE}/api/admin/transport/dashboard`, { headers: authHeader })
            .then((r) => r.json())
            .then((d) => {
                if (d.success) {
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((r) => !q || (r.college_name || "").toLowerCase().includes(q) || (r.college_code || "").toLowerCase().includes(q));
    }, [rows, search]);
    ...
    const handleCoordinate = async (id) => {
        setCoordinating(id);
        try {
            const res = await adminFetch(`${API_BASE}/api/admin/transport/coordinate/${id}`, {
                method: "PATCH", headers: authHeader,
            });
            const data = await res.json();
            if (!res.ok || !data.success) throw new Error(data.message || "Failed");
            const newStatus = data.data?.coordination_status;
            setRows((prev) => prev.map((r) => r.id === id ? { ...r, coordination_status: newStatus } : r));
            setSummary((prev) => ({
                ...prev,
    ...
    const handleDriverSaved = (id, savedData) => {
        setRows((prev) => prev.map((r) => r.id === id ? { ...r, ...savedData } : r));
    };

    const thStyle = (align = "left") => ({
        padding: "13px 14px", textAlign: align, color: "var(--text-muted)",
        fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px",
        whiteSpace: "nowrap", cursor: "pointer", userSelect: "none",
        background: "rgba(15,23,42,0.97)", borderBottom: "1px solid rgba(255,255,255,0.1)",
        position: "sticky", top: 0, zIndex: 2,
    });
    const tdStyle = (align = "left", extra = {}) => ({
        padding: "13px 14px", textAlign: align,
        borderBottom: "1px solid rgba(255,255,255,0.04)", verticalAlign: "middle", ...extra,
    });
    const sortIcon = (key) => {
    ...
    return (
        <AdminLayout>
            <div style={{ padding: "10px 0", display: "flex", flexDirection: "column", height: "100%" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "20px", flexWrap: "wrap", gap: "14px" }}>
                    <div>
                        ...
                        <p style={{ margin: "4px 0 0", color: "var(--text-muted)", fontSize: "0.82rem" }}>
                            Admin view — coordinate transport and assign drivers
                        </p>
                    </div>
                ...
                <div style={{ marginBottom: "16px", position: "relative" }}>
                    ...
                    <input
                        ...
                        style={{ width: "100%", boxSizing: "border-box", padding: "11px 16px 11px 40px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "10px", color: "#f1f5f9", fontSize: "0.92rem", outline: "none" }}
                    />
                    {search && (
                ...
                {loading ? (
                    ...
                    <div style={{ flex: 1, overflowY: "auto", overflowX: "auto", border: "1px solid rgba(255,255,255,0.09)", borderRadius: "14px", background: "rgba(255,255,255,0.03)", scrollbarWidth: "thin", scrollbarColor: "rgba(212,175,55,0.4) transparent" }}>
                                        ...
                                        <Fragment key={r.id}>
                                            ...
                                            >
                                                ...
                                                <td style={tdStyle("left")}>
                                                    ...
                                                    {r.college_code && <code style={{ color: "var(--text-muted)", fontSize: "0.72rem", background: "rgba(255,255,255,0.06)", padding: "1px 6px", borderRadius: "4px" }}>{r.college_code}</code>}
                                                </td>
                                                {/* Mode */}
                                                <td style={tdStyle("left")}><span style={{ color: "var(--text-secondary)", fontSize: "0.85rem" }}>{fmtMode(r.transport_mode)}</span></td>
                                                {/* Arrival */}
                                                <td style={tdStyle("left", { color: "var(--text-secondary)", fontSize: "0.85rem" })}>{r.arrival_point || "—"}</td>
                                                {/* ETA */}
                                                <td style={tdStyle("left", { color: "var(--text-muted)", fontSize: "0.78rem", whiteSpace: "nowrap" })}>{fmt(r.estimated_arrival_datetime)}</td>
                                                {/* Headcount */}
                                                <td style={tdStyle("center")}><span style={{ color: "#60a5fa", fontWeight: 700, fontSize: "0.95rem" }}>{r.total_headcount ?? "—"}</span></td>
                                                {/* Contact */}
                                                <td style={tdStyle("left")}>
                                                    <div style={{ color: "var(--text-primary)", fontWeight: 500, fontSize: "0.85rem" }}>{r.contact_person_name || "—"}</div>
                                                    {r.contact_person_phone && <a href={`tel:${r.contact_person_phone}`} style={{ color: "#60a5fa", fontSize: "0.78rem", textDecoration: "none" }}>{r.contact_person_phone}</a>}
                                                    {r.on_vehicle_contact_name && <div style={{ color: "var(--text-muted)", fontSize: "0.72rem", marginTop: "2px" }}>On-vehicle: {r.on_vehicle_contact_name}</div>}
                                                </td>
                                                {/* Status */}
                                                <td style={tdStyle("center")}><StatusBadge status={r.coordination_status} /></td>
                                                {/* Driver */}
                                                <td style={tdStyle("left", { maxWidth: "180px" })}>
                                                    {r.driver_name ? (
                                                        <div style={{ fontSize: "0.82rem" }}>
                                                            <div style={{ color: "#10b981", fontWeight: 600 }}>🚗 {r.driver_name}</div>
                                                            <div style={{ color: "var(--text-muted)", fontSize: "0.72rem" }}>📞 {r.driver_phone} · 🚌 {r.vehicle_number}</div>
                                                            {r.driver_remarks && <div style={{ color: "var(--text-muted)", fontSize: "0.7rem", fontStyle: "italic", marginTop: "2px" }}>{r.driver_remarks}</div>}
                                                        </div>
                                                    ) : (
                                                ...
                                                <td style={tdStyle("center")}>
                                                    <div style={{ display: "flex", gap: "5px", justifyContent: "center", flexWrap: "wrap" }}>
                                                        {/* Coordinate toggle */}
                                                        <button onClick={() => handleCoordinate(r.id)} disabled={isCoordinating}
                                                            style={{ padding: "5px 10px", borderRadius: "6px", cursor: isCoordinating ? "not-allowed" : "pointer", fontWeight: 700, fontSize: "0.72rem", whiteSpace: "nowrap", background: isCoordinated ? "rgba(245,158,11,0.12)" : "rgba(16,185,129,0.12)", border: `1px solid ${isCoordinated ? "#f59e0b" : "#10b981"}`, color: isCoordinated ? "#f59e0b" : "#10b981", opacity: isCoordinating ? 0.6 : 1, transition: "all 0.2s" }}>
                                                            {isCoordinating ? "…" : isCoordinated ? "↩ Uncoordinate" : "✓ Coordinate"}
                                                        </button>
                                                        {/* Note */}
                                                        <button onClick={() => setNoteOpen(isNoteOpen ? null : r.id)}
                                                            style={{ padding: "5px 10px", borderRadius: "6px", cursor: "pointer", fontWeight: 700, fontSize: "0.72rem", whiteSpace: "nowrap", background: isNoteOpen ? "rgba(212,175,55,0.18)" : "rgba(212,175,55,0.08)", border: "1px solid rgba(212,175,55,0.5)", color: "#d4af37", transition: "all 0.2s" }}>
                                                            {r.internal_note ? "✏ Note" : "+ Note"}
                                                        </button>
                                                        {/* Driver */}
                                                        <button onClick={() => setDriverModal({ open: true, rowId: r.id, initialData: { driver_name: r.driver_name, driver_phone: r.driver_phone, vehicle_number: r.vehicle_number, driver_remarks: r.driver_remarks } })}
                                                            style={{ padding: "5px 10px", borderRadius: "6px", cursor: "pointer", fontWeight: 700, fontSize: "0.72rem", whiteSpace: "nowrap", background: r.driver_name ? "rgba(167,139,250,0.18)" : "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.5)", color: "#a78bfa", transition: "all 0.2s" }}>
                                                            {r.driver_name ? "🚗 Edit Driver" : "🚗 Assign Driver"}
                                                        </button>
                                                    </div>
                                                </td>
                                            </tr>
                                            {isNoteOpen && (
                                                <NoteEditor row={r} onSaved={handleNoteSaved} onClose={() => setNoteOpen(null)} />
                                            )}
                                        </Fragment>
                                    );
                                })}
                ...
                {!loading && sorted.length > 0 && (
                    <div style={{ marginTop: "10px", color: "var(--text-muted)", fontSize: "0.78rem", textAlign: "right" }}>
                        Showing {sorted.length} of {rows.length} submissions{search && ` · filtered by "${search}"`}
                    </div>
                )}
            </div>
            ...
            <DriverAssignModal
                ...
                onSuccess={(savedData) => { handleDriverSaved(driverModal.rowId, savedData); }}
                apiUrl={`${API_BASE}/api/admin/transport/driver`}
                fetchFn={adminFetch}
