...
import { isVolunteerTokenExpired } from "../../utils/volunteerFetch";

export default function IDCardLayout({ children }) {
  const navigate = useNavigate();
  const location = useLocation();
  const role = localStorage.getItem("vtufest_idcard_role");
  const name = localStorage.getItem("vtufest_idcard_name") || "Volunteer";

  const isEditor = role === "id_card_editor";

  const NAV_ITEMS = isEditor
    ? [{ path: "/media/editor", label: "ID Card Editor", icon: "🪪" }]
    : [{ path: "/media/team", label: "ID Card Team", icon: "📦" }];
  ...
  const doSessionExpiry = () => {
    ...
    navigate("/media-login", { replace: true });
  };

  useEffect(() => {
    const token = localStorage.getItem("vtufest_idcard_token");
    if (!token || isVolunteerTokenExpired()) { doSessionExpiry(); return; }
    const handler = () => doSessionExpiry();
    window.addEventListener("idcard:session-expired", handler);
    return () => window.removeEventListener("idcard:session-expired", handler);
  }, []);
  ...
  const roleLabel = isEditor ? "PHOTO EDITOR" : "ID CARD TEAM";
      ...
      <aside style={{
        ...
        <div style={{ padding: "24px 20px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <div>
              ...
              <div style={{ color: accentColor, fontSize: "0.75rem", fontWeight: 600 }}>ID Card Portal</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <div style={{
            ...
            background: "rgba(129,140,248,0.2)", color: accentColor,
            border: `1px solid ${accentColor}`, letterSpacing: "0.5px",
          }}>
            {roleLabel}
          </div>
        </div>
      ...
      <main style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{
          ...
          <h2 style={{ margin: 0, color: "#f1f5f9", fontSize: "1.1rem", fontWeight: 600 }}>
            ...
            {NAV_ITEMS.find((n) => n.path === location.pathname)?.label || "ID Card Portal"}
          </h2>
          <div style={{ color: accentColor, fontSize: "0.8rem", fontWeight: 600 }}>{name}</div>
        </div>
